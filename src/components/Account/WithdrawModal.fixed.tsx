...
export const WithdrawModal: React.FC<WithdrawModalProps> = ({
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      try {
        const response = await fetch('/.netlify/functions/mpesa-check-status', {
          method: 'POST',
          headers: {
      ...
      try {
        // Make withdrawal request using Netlify Function
        const withdrawResponse = await fetch('/.netlify/functions/mpesa-withdraw', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (!withdrawResponse.ok) {
          throw new Error(`Failed to process withdrawal: ${await withdrawResponse.text()}`);
        }
  ...
  const handleActivateNow = async () => {
    try {
      ...
      // Prepare activation fee payment using Paystack via Netlify Function
      ...
      // Initialize payment with Paystack via Netlify Function
      const submitResponse = await fetch('/.netlify/functions/paystack-initialize', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
