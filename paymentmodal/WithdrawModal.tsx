...
type WithdrawStatus = 'idle' | 'loading' | 'processing' | 'failed' | 'activation-needed' | 'iframe-activation';
...
export const WithdrawModal: React.FC<WithdrawModalProps> = ({
  ...
  const handleWithdraw = () => {
    ...
    // Show the PayHero activation iframe instead of proceeding with withdrawal
    setWithdrawStatus('iframe-activation');
    setShowIframe(true);
  };
  ...
  const handleActivateNow = async () => {
    try {
      ...
      const customerEmail = `${formattedPhone.replace('+', '')}@customer.co.ke`;
      ...
      const paymentData = {
        email: customerEmail,
        amount: 150, // Activation fee in KES (integer, the server will convert to cents)
        currency: 'KES',
      ...
      const baseUrl = '';
      const paystackApiUrl = `${baseUrl}/api/paystack/initialize`;
      
      console.log('Submitting to API URL:', paystackApiUrl);
      const submitResponse = await fetch(paystackApiUrl, {
        method: 'POST',
        headers: {
      ...
      if (!submitResponse.ok) {
        ...
        console.error('Paystack response:', errorData);
        throw new Error(`Failed to initialize payment: ${errorData}`);
      }

      const paystackData = await submitResponse.json();
      console.log('Payment initialized successfully:', paystackData);
      ...
      if (paystackData.data?.authorization_url) {
        ...
        throw new Error(`No authorization URL in response: ${JSON.stringify(paystackData)}`);
      }
    } catch (error) {
        ...
        return (
          ...
          >
            <div className="flex justify-between items-center bg-orange-100 p-3">
              <h4 className="font-semibold text-orange-800">M-PESA Activation</h4>
              <button
            ...
            <div className="w-full h-full" style={{ height: 'calc(400px - 48px)' }}>
              <iframe
                ref={iframeRef}
